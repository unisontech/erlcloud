...
-export([aws_request/5, aws_request/6, aws_request/7, aws_request/8,
         ...
         aws_request_json4/6,aws_request_json4/8,
         aws_request_form/8,
         param_list/2, default_config/0, update_config/1, format_timestamp/1,
         http_headers_body/1,
         request_to_return/1,
         sign_v4/5, sign_v4/7, sign_v4/8]).
...
aws_request_json4(Method, Host, Path, Params, Service, #aws_config{} = Config) ->
    aws_request_json4(Method, undefined, Host, undefined, Path, Params, Service, Config).
aws_request_json4(Method, Protocol, Host, Port, Path, Params, Service, #aws_config{} = Config) ->
    case aws_request4j(Method, Protocol, Host, Port, Path, Params, Service, Config) of
        {ok, Body} ->
            {ok, jsx:decode(Body)};
        {error, Reason} ->
            {error, Reason}
...
aws_request4_no_update(Method, Protocol, Host, Port, Path, Params, Service, #aws_config{} = Config) ->
    ...
    SignedHeaders = case Method of
        get  -> sign_v4(Method, Config, Headers, Params, "", Region, Service, Path);
...
aws_request4j(Method, Protocol, Host, Port, Path, Params, Service, Config) ->
    case update_config(Config) of
        {ok, Config1} ->
            aws_request4j_no_update(Method, Protocol, Host, Port, Path, Params, Service, Config1);
        {error, Reason} ->
            {error, Reason}
    end.

aws_request4j_no_update(Method, Protocol, Host, Port, Path, Params, Service, #aws_config{} = Config) ->
    QueryToSign = case Params of
        {_, B} -> B
        ;_     -> erlcloud_http:make_query_string(Params)
    end,
    ...
    SignedHeaders = case Method of
        get  -> sign_v4(Method, Config, Headers, Params, "", Region, Service, Path);
        post -> sign_v4(Method, Config, Headers, "", QueryToSign, Region, Service, Path)
    end,

    aws_request_json(Method, Protocol, Host, Port, Path, QueryToSign, SignedHeaders, Config).

-spec aws_request_json(Method :: atom(), Protocol :: undefined | string(), Host :: string(),
                        Port :: undefined | integer() | string(), Path :: string(), Form :: iodata(),
                        Headers :: list(), Config :: aws_config()) -> {ok, binary()} | {error, tuple()}.
aws_request_json(Method, Protocol, Host, Port, Path, Form, Headers, Config) ->
    ...
    Response =
        case Method of
            get ->
                URL1 = case URL of
                    [["https","://"],Th,58,"443",Tp] -> ["https://",Th,Tp]
                    ;_ -> URL
                end,
                Req = case Form of
                    %[] -> lists:flatten([URL1,"/"]) 
                    [] -> lists:flatten([URL1]) 
                    ;_ -> lists:flatten([URL1, $?, Form])
                end,
            ...
            _ ->
                ...
                Form1 = case is_binary(Form) of
                    true -> Form;_ -> list_to_binary(Form)
                end,
                erlcloud_httpc:request(
                  lists:flatten(URL1), Method, 
                  [{<<"content-type">>, <<"application/json; charset=utf-8">>} | Headers],
                  Form1, Config#aws_config.timeout, Config)
        end,
...
-spec sign_v4(atom(), aws_config(), headers(), string(), iodata(), string(), string(), string()) -> headers().
sign_v4(Method, Config, Headers, QueryParams, Payload, Region, Service, Uri) ->
    ...
    {Request, SignedHeaders} = canonical_request(MethodString, Uri, CanonicalQueryString, Headers2, Payload),
